...
class ElectronicDiary:
    def __init__(self, login, password, score=4.60):
        self.login = login
        self.password = password
        self.url_logon = 'https://edu.tatar.ru/logon'
        self.params = {
            'main_login2': self.login,
            'main_password2': self.password
        }
        ua = UserAgent()
        random = ua.random
        self.score = score
    ...
    def registration(self):
        response_logon = requests.post(self.url_logon, params=self.params, headers=self.head)
        cookie = response_logon.cookies
        return cookie

    def get_schedule_for_today(self):
        cookie = self.registration()
    ...
    def get_num_fours(self):
        try:
            ...
            for key, value in dictionary.items():
                ...
                if average_score < self.score:
                    sp = value.copy()
                    while sum(sp) / len(sp) < self.score:
                        sp.append(5)
